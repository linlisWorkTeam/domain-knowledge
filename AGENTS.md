...
- 本仓库的 commit author、committer 和新建 PR 的 GitHub 账号必须统一为 `icedblkamericano`。此项目规则覆盖旧交接中的账号约定。
- 在仓库级配置 `user.name=icedblkamericano`、`user.email=46128748+icedblkamericano@users.noreply.github.com`；不要通过修改全局 Git 配置影响其他项目。
- 提交前运行 `git var GIT_AUTHOR_IDENT` 和 `git var GIT_COMMITTER_IDENT`；创建或更新 PR 前使用该操作实际凭据执行 `gh api user --jq .login`，必须返回 `icedblkamericano`，不得用其他账号代发。
- 如当前登录不符，使用已授权的该账号凭据；不得在日志、PR 或文件中输出 token。没有可用凭据时明确报告身份阻塞，不伪装作者。
- 历史提交只有用户明确要求才重写；更新远端使用带预期旧 tip 的 `--force-with-lease`，不重写 main 或无关分支。GitHub 已有 PR 创建者不可直接修改。
