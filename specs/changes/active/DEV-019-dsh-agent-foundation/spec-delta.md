...
生产默认 bubblewrap，受控模型传输测试显式不使用进程隔离，权限边界另由 T102 原生工具与工作区测试证明。本地 SSE 服务驱动真实 DSH 七节点及独立 CPU 评测，只证明机制。R2 后续已用真实模型完成单 DocGen 与独立复现（见 evidence.md）；R3/R4 live 仍为 NOT_RUN。CodeAgent CLI 仅保留后置 Adapter，不计为已验证集成。
