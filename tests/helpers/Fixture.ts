...
export async function acceptedCandidate(composition: ReturnType<typeof createTestComposition>, suffix = '') {
  return composition.service.ingestCandidate({
    ...
    provenance: [{ path: 'docs/specs/domainFunction/services/evaluation/Evaluation.md', commit: 'abc123', pinned: true }],
  });
}

export const GENERIC_SCENARIO = {
  schemaVersion: '1.0' as const, name: 'cpu-module', moduleId: 'cpu-module', repositoryRoot: process.cwd(),
  sourcePaths: ['src/domain/services/knowledge/MarkdownDiff.ts'], publicInterfacePaths: [],
  allowedGeneratedPaths: ['src/domain/services/knowledge/MarkdownDiff.ts'], prepareCommands: [],
