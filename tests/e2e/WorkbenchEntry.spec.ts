...
test('workbench introduces the five-stage task before historical issues on desktop and narrow screens', async ({ page }) => {
  ...
  try {
    ...
    await expect(page.getByRole('heading', { name: '从代码仓开始知识任务' })).toBeVisible();
    ...
    expect(await page.locator('[data-repository-panel]').evaluate(element => {
      const history = document.querySelector('.workbench-history-heading')!;
      return Boolean(element.compareDocumentPosition(history) & Node.DOCUMENT_POSITION_FOLLOWING);
    })).toBe(true);
