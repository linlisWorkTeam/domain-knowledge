...
import { type StageInput, type StageResult, type JsonValue } from '../../src/domain/services/workbench/StageTask.ts';
...
test('fixed pipeline pauses before source on failed fixed cases, resumes the same child and freezes changed suites separately', async () => {
  ...
  const open = () => {
    ...
    const app = new WorkbenchPipelines({ artifacts, store, stages, materials: { get: () => null }, environment: async () => 'environment',
      ...
      fixedEvaluation: { prepare: async (id, suites) => {
        ...
        const ref = await artifacts.put(Buffer.from(JSON.stringify(suites[0]!.suite)), 'application/json');
  ...
  try {
    ...
    assert.notEqual(corrected.pipelineId, first.pipelineId); assert.equal((await runtime.app.wait(corrected.pipelineId)).status, 'SUCCEEDED');
    ...
    assert.equal(runtime.app.detail(corrected.pipelineId).publicationVerified, false);
