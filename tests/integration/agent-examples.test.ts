...
import { AGENT_IDS } from '../../src/domain/agents/contracts.ts';
import { main } from '../../src/interfaces/runner/agent-run.ts';
...
for (const role of AGENT_IDS) test(`standalone ${role} sample commits through the production role and records one stage`, async () => {
  ...
  try {
    const result = await main(['--role', role, '--input', `src/domain/agents/${role}/examples/sample.json`, '--output', directory]);
...
test('standalone invalid output records failure and retains the failed Run for inspection', async () => {
  ...
  try {
    const sample = JSON.parse(readFileSync('src/domain/agents/code/examples/sample.json', 'utf8'));
