...
import { buildDemoReport } from '../../src/interfaces/runner/DemoReport.ts';

for (const provider of ['deepseek-harness', 'pi-agent']) {
  test(`demo report exports authoritative ${provider} run facts and allowlisted audit fields only`, async () => {
    ...
    const startedAt = '2026-09-02T00:00:02.000Z';
    const completedAt = '2026-09-02T00:00:03.000Z';
    appendFileSync(join(auditDir, 'agent-runs.jsonl'), `${JSON.stringify({
      provider, role: 'doc-gen', idempotencyKey: 'PI_KEY_MUST_NOT_LEAK',
      workspaceRoot: '/PI_PATH_MUST_NOT_LEAK', promptSha256: 'c'.repeat(64), schemaSha256: 'd'.repeat(64),
      startedAt, completedAt, durationMs: 1000,
    ...
    metrics.recordProviderInvocation({
      ...
      provider, model: 'MODEL_BODY_MUST_NOT_LEAK',
      startedAt, completedAt, durationMs: 1000,
    ...
    try {
      ...
      const providerCalls = calls.filter((call) => call.provider === provider);
      assert.equal(providerCalls.length, 2);
      const invocation = providerCalls.find((call) => (call.tokens as { total?: number })?.total === 20);
      assert.deepEqual(invocation, {
        provider, role: 'doc-gen',
        startedAt, completedAt, durationMs: 1000,
        status: 'FAILED', retryCount: 1,
