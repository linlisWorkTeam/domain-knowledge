...
import type { ProviderSettingsRecord } from '../../src/application/ports/ApplicationPorts.ts';
import {
  EncryptedFileProviderSettingsStore, isPublicAddress, PublicHttpsEndpointPolicy,
} from '../../src/infrastructure/agentAdapters/provider/ProviderSettings.ts';
...
test('Provider probe connects only to its approved address with Node family autoselection enabled', async () => {
  ...
  const { OpenAiCompatibleProviderProbe } = await import('../../src/infrastructure/agentAdapters/provider/ProviderSettings.ts');
  let connected = false;
