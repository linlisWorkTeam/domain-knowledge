...
旧的 Provider 和 DSH Web 调试 YAML 已移除。模型接入由 `src/infrastructure/agentAdapters/deepSeekHarness/` 维护，OpenCode Go 通过上文的环境配置生成运行文件；默认原生 DeepSeek / Console 路径不加载 OpenCode Go 的模型插件。
