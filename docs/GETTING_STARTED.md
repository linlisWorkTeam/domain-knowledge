...
如果新建了 Git worktree，应在其中执行 `npm run bootstrap:worktree` 安装依赖，确认结果为 `status: READY`；后续用 `npm run bootstrap:worktree:check` 检查环境。每个 worktree 使用独立的 `node_modules`，详见[开发指南](DEVELOPMENT.md#开发基线)。
...
运行 DSH 需要 Linux/Bubblewrap 及有效模型配置，来源目录须在 `WP_DSH_ALLOWED_ROOTS` 中；部署步骤见 [DSH 部署说明](guides/dsh-runtime.md)。R1 自动化使用受控模型服务，真实模型范例由 R2 单独验收。
...
2. 阅读 AGENTS.md、最新 docs/epitaph/ 交接、README.md、CONTRIBUTING.md、docs/GETTING_STARTED.md 和 specs/README.md。交接只作为上下文，以本次请求为准。
