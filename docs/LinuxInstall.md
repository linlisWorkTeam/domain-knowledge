...
评测详情会将“知识风险尚未解决”与“代码检查存在阻塞”分别显示。证据工件中的 `knowledge-risk-assessment-v1` 记录该版本每项风险的原始来源、复核结果和依据；已验证仅指固定及晋升案例，范围外限制仍是未验证内容。普通缺证据风险不会被 Review 的 PASS 自动清除，需要补齐材料并重新提取。

风险复核使用 `seven-role-mvp-v4` 执行契约；旧 v3 运行继续可读，但不能恢复。已经耗尽轮数或时间的运行不会因升级重新获得预算。本次代码修复尚未替换历史安装候选，也不表示新增真实模型验收已通过；当前交付状态见 [Status](Status.md)。


## 显式额度模式验收

用户明确授权“不限轮数，token 用完为止”时，验收入口接受新的摘要绑定授权：executionMode 为 provider-quota，包含 authorizationId、approvedAt 和 previousLedgerSha256。该授权把账本扩展为 mvp-real-attempts-v3，完整保留已有启动记录与其摘要；不得手改旧次数。CLI继续使用 `--authorization <授权文件>`，文件中不包含密钥。

额度模式没有全局轮数与30分钟墙钟截止，仍受每个角色阶段、网络及子进程超时、输出大小、隔离与取消约束。达到确定性门禁即结束，不为耗尽额度而继续运行。普通浏览器启动默认仍为3轮/30分钟。当前执行版本v5，旧版本运行只读不可恢复。

提供方明确返回额度机器码时停止；HTTP402作为支付拒绝停止，不能直接声称token精确用尽。普通429只表示限流，不无限重试。额度/支付停止标记保存在私有DSH数据目录，跨角色、配置revision与进程重启生效；补充额度后需重新确认并处理停止状态，应用不自动解除。不会从累计usage推算未知订阅余额。停止原因与验收结果均留证。

## 前台免登录（源码更新）

当前源码支持 `WP_KNOWLEDGE_NO_LOGIN=1`：启动服务时设置后，浏览器可直接配置模型、选择目录及操作批次，无需治理令牌。启用后公网链接同样可编辑；目录范围和密钥脱敏保持有效。未启用时，无令牌的直接本机访问也可编辑，远程令牌方式保留。此变更尚未重打 v0.2.0 安装包，上述已发行版本说明仍描述旧包行为。当前本地预览的启动脚本已由 Agent 配置，无需用户编辑环境文件。设计见 [界面规则](specs/totalRules/UiuxDesign.md)。
