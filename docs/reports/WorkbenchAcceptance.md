...
新增source-historical-review-v1显式策略与“重新核实历史意见”入口。该策略形成独立任务，原检查点和Review不改写；新Review逐条确认、反驳或保留未知，必须给出理由，反驳需固定源码引用。默认复核仍继承原矛盾；格式修正仍受同任务事实保留约束。发布准备和恢复重验原检查点、正文/源码/命令绑定，发布逐条核对新回应与冻结线索。页面显示中文核实结论与理由。

31项定向回归、4项完整原生流程、5项架构检查、类型与Spec通过；页面补充中文回应后5项面板回归通过。原生测试覆盖默认继承不变、显式新Review、遗漏历史回应拒绝、确认意见仍保留矛盾及原任务不变。发布测试使用受控反驳，验证原证据篡改会使准备和恢复失败，不代表真实来源通过。

真实43例评测的准备检查已识别1条旧意见（ToInt接口声明与调用约定），并冻结策略和原证据，记录 `cpp-v5-reference-supplement/HistoricalReviewPreparation.json`。此时尚未启动模型任务。源码引用存在仍不等于推理正确；完整C/C++来源与最终发布、删除兼容及线上完整验收尚未完成。

## 补证策略的发布校验修复（2026-09-15）

检查43例补证的后续链路发现：测试集记录包含补证需求与候选摘要的策略身份，但发布准备只计算普通评测身份，会误拒绝合法补证。已让执行与发布共用策略摘要计算，并让发布重新读取原来源任务、固定正文与需求，校验冻结目标及候选。指定候选必须完整出现在可信用例集，允许合并时更名caseId，不允许更改其他字段。

28项发布证据/来源准备/候选校验回归、类型、Spec及架构检查通过。新增测试覆盖普通补证、指定候选补证、退回普通策略、篡改原需求和可信集中缺少候选的拒绝。测试构造的来源PASS仅用于验证发布校验，不代表真实C++来源已通过。本轮没有运行新模型、修订卡片、发布知识或更新网站；旧意见重新复核仍待实现。

## C++正式补证通过（2026-09-15）

候选入口20e7949的CI34869515211已全部通过729项代码检查、44项Console测试及25项隔离验收。真实补证任务 `stage-28fc7aabdb6139b7093c3264cc2db212a206f8d77a8b09b6f57938e56a3c8082` 已SUCCEEDED：复用37个历史可信用例，新增6个ToInt边界用例，固定参考复验43/43、重建评测43/43。可信测试集为 `native-tests-19256226c6dafd55ff8289c4bf53f0148dd4c0fed962aeae8ba643877211d9cd`；原37例逐项输入、观察定义、预期及章节引用完全不变，103个直接工件引用通过大小与摘要校验。全程0模型调用，累计执行211007ms。证据在 `cpp-v5-reference-supplement/Evaluation.json` 和 `Audit.json`，替代下节“尚未晋升”的当时状态。

首轮及192MiB堆恢复因资源不足暂停，3个检查点保留；最终使用128MiB堆与显式GC的验收驱动恢复同一任务成功，编译内存、进程和隔离门槛未变。一次jitless尝试因Node TypeScript加载器需要WebAssembly而在加载前退出，未恢复任务；有效日志为 `/tmp/CppV5ReferenceSupplementResume3.log`。未停止其他服务或删除数据。不能把这次驱动设置变更误写为生产配置变更。

6例直接定位ToInt适用条件章节，其余26个补证章节仍无新增直接覆盖。来源434571原结论没有改写，新来源复核尚未启动；尤其旧接口SOURCE_MISMATCH的无条件继承仍需处理，不能因行为通过自动抹去或确认静态意见。本轮未发布知识、未更新线上956fe26，也未导入临时验收数据。

## ToInt声明式反证执行（2026-09-15）

使用现有NativeCaseExecutor、NativeToolchain和Domain观察比较，按冻结源码、接口及构建参数逐个隔离执行6个声明式候选：0x、0X、0xg、0x10、前导空白加0x、junk。每例先在固定参考实现通过，再执行当前cf781重建代码。两侧均6/6通过；裸前缀返回true并写0，0x10写16，junk返回false且保留初值123。本轮无模型调用，执行进程正常退出。

证据 `cpp-source-v5/HexPrefixDeclarative.json`保存完整声明式用例、两侧编译运行报告、参考/生成/接口CAS引用及冻结构建引用；日志 `/tmp/CppHexDeclarative.log`。脚本读取实际阶段库确认无RUNNING任务，并验证输入卡片版本与源码摘要及CAS大小/摘要。该证据进一步排除这6例的重建行为差异，不证明其他输入或所有失败路径。

这些用例尚未晋升为持久化可信测试集，也未通过工作台阶段接口导入来源复核，原37项可信预期未变。下一步需要实现可审计的参考观察交接和Review反证处理；不能把阶段外检查冒充正式补证任务，不能恢复已成功的434571任务或直接发布。

## 来源v5保留待核实线索（2026-09-14）

来源v5冻结pendingConcernsRef，按源码、原任务与事件、卡片版本和章节验证旧失败输出。线索不自动升级为纠正意见；新Review须逐条回答CONFIRMED、DISPROVED或UNRESOLVED。否定需要固定源码中确实存在的引用，发布重新检查线索与回应，不允许遗漏。原v4任务及用量保留，不跨契约恢复；同v5父评测重复启动复用冻结输入。

50项来源/发布/角色定向回归、类型与Spec通过。额外5项准备与绑定测试覆盖v4/v5隔离及线索文字、卡片版本、原工件篡改。只读真实历史读取0条已授权继承意见、2条待核实线索，记录cpp-current-contract-source/PendingConcernsAudit.json；引用匹配只证明出处，不自动证明Review的推理正确。

新任务 `stage-434571dde4545602e48f0c9fdc15e7ad1d473f93b1759819825c36f2f024abf7` 绑定原eb7e2c可信评测，冻结原9张卡片和2条线索，正在运行；证据cpp-source-v5/Source.json，日志/tmp/CppSourceV5.log。首次“用途与定位”回应已CONFIRMED原内联位置错误并保留完整修订，但同时保留空指针场景未知风险，章节整体仍UNRESOLVED。不能将此视作问题已修正或知识已发布；应在不清除其他风险的前提下推进已确认差异的修订。
...
v5来源任务434571首轮在22调用、1460841 tokens后因REVIEW_CONCERN_UNRESOLVED失败。核对失败原输出及固定checkReportRef发现：最后一次回应的5段引用均存在于固定源码；3段多行引用仅CRLF/LF不同，并非虚构语句。直接引用122份的字节数与摘要已核对，记录在cpp-source-v5/Audit.json。没有修改旧输出或放行失败结果。e81ce62仅补充换行符不一致的语义反馈，要求保留原结论、精确复制换行符或选择足够支持理由的单行源码；原逐字校验、语义尝试上限及发布规则不变。16项角色/发布测试、类型和Spec通过。确认原PID274903退出且无RUNNING任务后，用原任务ID恢复，保留22调用和61个检查点；当前运行日志CppSourceV5Resume1.log，尚未完成修订或发布。

9eb8470修复prepare过早复用成功来源任务的回归：活动/失败任务保持冻结，完成后新明确矛盾可冻结后续输入。原CI三项断言不改，6项准备测试、类型/Spec通过；本地原生集成被内存预检阻止，未降低门槛，远端CI34863754128仍待结果。

### 来源 v5 前台契约同步

CI34864023225代码检查725/725通过，Console43/44；失败原因是KnowledgeSourceVerification.js仍把v4识别为当前契约，隐藏了v5的补充用例入口。前台已同步v5，并拒绝旧契约的遗留操作事件。6项面板测试与完整7步工作台浏览器流程通过，Spec及diff检查通过；原浏览器断言未改。单独筛选最后一步缺少串行前置项目，故改为运行完整组，日志SourcePanelV5BrowserSuite.log。尚未部署本次修复。

真实C++来源任务434571已恢复至第3次任务尝试，日志CppSourceV5Resume2.log，检查时42调用2668531tokens仍RUNNING。已越过裸十六进制前缀所在章节，该章节保持UNRESOLVED。模型关于裸0x失败的意见与主机libc观察相反，HexPrefixDiagnostic.json明确仅是主机诊断；已准备VerifyCppHexPrefix.ts，按冻结CAS源码及原构建参数隔离验证，AfterCppSourceReference.py等待模型进程结束后执行，不修改可信测试集或发布。正式参考验证尚未执行。

### 格式拒绝记录与恢复约束

来源v5在59调用、3891825tokens后FAILED/REVIEW_REPAIR_FACTS_CHANGED；162检查点包含51章节（30匹配、20未解决、1明确差异），307直接引用校验通过。固定CAS源码与原构建参数的隔离诊断随后成功：TinyXML2::XMLUtil::ToInt对裸0x/0X/0xg返回true并写0，0x10写16，junk返回false。HexPrefixReference.json保留编译和运行结果，不能按Review提出的错误裸前缀失败预期修订，也未修改已有可信测试集。

恢复修复：WorkbenchRoleExecution提供独立只读history，Review恢复首请求携带原格式失败判断；当前尝试计数与截止时间仍使用read。来源历史审计不将有事件和工件共同证明为REJECTED/REVIEW_REPAIR_FACTS_CHANGED的输出当成可用结果，原格式失败及全部PASSED结果仍按原约束核对。37项角色/发布/恢复回归与4项原生完整集成通过，类型/Spec通过；原矛盾PASS断言未改。真实v5的644事件只读审计可继续恢复，旧v4错误PASS任务67事件仍拒绝。审计不修改任务、工件或累计用量。
