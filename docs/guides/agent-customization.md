...
本指南分为目标开发方式与现有定制操作。2026-09-07 已确认先完成 DSH 公共底座，再由各负责人开发七个角色；目标开发方式尚未实现。现有 Console 的 `promptAddon` 仅用于调整已固定角色的表达，不能代替完整的角色开发。
...
底座和范例明确后，基于同一 SOP 按职责分发七角色开发；输入、交付和验收写回现有 `specs/06-agents/` 对应章节，不再新建七份任务书。执行顺序与阶段验收见 [DEV-019 Roadmap](../../specs/changes/active/DEV-019-dsh-agent-foundation/plan.md)，下一步从 R0 核实基线并进入 R1 实施。常规文件拆分和接线选择由实施者按实际能力落实；CodeAgent CLI 仅保留后续适配位置，不要求角色开发者当前实现双后端。

## 当前实现与操作说明

以下说明当前已存在的固定角色及 `promptAddon` 操作。Pi 和固定项目执行器仍在代码中，属于需要迁出的现状，不是后续开发者应复制的目标方案。
