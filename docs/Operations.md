...
在“飞轮批次”点击新建批次，只选择当前项目中的模块。自动运行选“否”时仅创建记录，点击“新增轮次”启动；选“是”时填写分钟频率。相同模块串行，不同模块可同时排队推进，实际模型与构建仍受全局资源限制。编号为模块-日期-序号。选择模块与批次后切换轮次，展开节点图或右侧日志查看时间、事件和证据；失败时恢复本轮保留原用量。定时批次遇到质量暂停不会自动绕过。当前新批次子页尚未提供固定测试及外部材料选择，需使用上述项目任务面板完成完整发布验收。

当前代码与受控测试已覆盖这些入口；网站已于2026-09-14更新到5c72fa5，本机及公网健康检查通过，部署后的桌面、390px窄屏入口及原知识详情只读浏览器复查通过；新C/C++完整阶段页面验收仍待完成。线上保留原数据，独立运行库中的真实验收任务尚未导入。jsmn、TinyXML2 的真实最新来源修订/最终发布及当前完整回归仍未完成。现场验收要分别记录运行编号、最终版本、固定与可信用例、来源意见、发布ID和文件校验，不能只用阶段成功或脚本能力替代结果。保留旧v0.2.0发布。

### 补充验证与任务恢复

整卡来源复核保留未知段落时，可点击“补充验证用例”。新任务冻结这些段落，先保留并验证原可信测试，再提出新候选。候选没有引用任何指定段落时，不执行候选测试；页面列出未命中段落。引用命中只用于定位，不能代表该段落已通过语义验证。

候选被拒绝后，使用“重新生成候选测试”恢复原任务。已拒绝的输入、参考观察或目标缺口会交给TestGen，累计用量继续计入。旧可信输入与预期不修改；补证通过后仍须重新复核来源。对已存在的旧策略补证任务，点击启动或恢复保留原身份与策略，不会创建同输入新任务来重置预算。
