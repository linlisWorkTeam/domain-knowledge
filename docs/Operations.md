...
有明确矛盾时可「按来源意见修订」，随后查看前后正文和增量索引结果，再「重建修订版本」、执行评测和整卡复核。来源修订不伪造行为失败、不修改可信预期。索引失败后恢复原任务，不重复生成已接受正文。这些操作既可独立执行，也由一键v10流程调用。来源复核v2逐章保存进度，恢复不会重跑已完成章节；旧v1和旧v9流程只读。固定测试与工作台发布事务仍待接通，完成来源匹配也不代表已发布。
...
仓库分析会读取固定提交中的 `compile_commands.json`，在“编译数据库候选”展开来源、编译参数和未支持项。无问题的候选可“应用到构建参数”，再调整并保存项目输入；当前表单参数应用于全部所选模块，不同模块的独立配置尚未接通。系统不会执行数据库中的命令。
