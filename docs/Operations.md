...
当前代码与受控测试已覆盖这些入口；网站已于2026-09-14更新到f543c59，本机及公网健康检查通过，部署后的桌面、390px窄屏入口及原知识详情只读浏览器复查通过；新C/C++完整阶段页面验收仍待完成。线上保留原数据，独立运行库中的真实验收任务尚未导入。jsmn、TinyXML2 的真实最新来源修订/最终发布及当前完整回归仍未完成。现场验收要分别记录运行编号、最终版本、固定与可信用例、来源意见、发布ID和文件校验，不能只用阶段成功或脚本能力替代结果。保留旧v0.2.0发布。
