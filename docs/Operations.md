...
有明确矛盾时可「按来源意见修订」，随后查看前后正文和增量索引结果，再「重建修订版本」、执行评测和整卡复核。来源修订不伪造行为失败、不修改可信预期。索引失败后恢复原任务，不重复生成已接受正文。这些操作既可独立执行，也由一键v15流程调用。来源复核逐章保存进度，恢复不会重跑同版本已完成章节；旧契约只读。固定测试与工作台发布事务已接通，但只有发布提交成功后才标记已验证，来源匹配本身不等于已发布。

真实验收脚本 `scripts/release/RunNativeWorkbench.ts` 的 steps 模式在行为通过后执行同一个来源复核入口，明确矛盾或未知风险返回失败并保留任务编号；处理后再继续。pipeline 模式使用当前v10自动修订流程，旧契约不得恢复。二者通过均标记 `BEHAVIOR_AND_SOURCE_PASSED_PUBLICATION_PENDING`，不冒充最终发布。
...
1. 在操作中心选择已保存的仓库/源码输入，或填写目录与版本后分析。确认模块及构建参数，保存冻结快照。切换快照会读取对应的任务历史，后台原任务保留。
2. 点击“生成知识库”查看可立即阅读的卡片，再到知识页更新索引、预览 YAML 并试检索。生成完成不表示已验证。接口编译失败时查看具体缺项和下载诊断；修正参数后保存新的输入。
...
6. 独立发布选择本轮可信、固定和来源任务，点击“验证并发布知识”；一键v15流程在关联完成后自动调用同一发布事务。只有提交成功才显示“已验证并发布”，卡片、清单和证据可下载。导出失败保留前序产物和待恢复记录；恢复不会重跑成功模型任务。没有固定用例的流程不具备最终发布资格。

当前代码与受控测试已覆盖这些入口；jsmn、TinyXML2 的真实最新来源修订/最终发布、完整浏览器检查和当前网站更新仍未完成。现场验收要分别记录运行编号、最终版本、固定与可信用例、来源意见、发布ID和文件校验，不能只用阶段成功或脚本能力替代结果。保留旧v0.2.0发布。
