...
当前代码与受控测试已覆盖这些入口；网站已于2026-09-11部署v17，之后的补证目标检查尚未部署。jsmn、TinyXML2 的真实最新来源修订/最终发布及当前完整回归仍未完成。现场验收要分别记录运行编号、最终版本、固定与可信用例、来源意见、发布ID和文件校验，不能只用阶段成功或脚本能力替代结果。保留旧v0.2.0发布。
