...
在操作中心填入允许目录内的 Git 仓库根目录，以及分支、标签或提交（默认 HEAD），点击“分析仓库”。返回的完整提交号和源码清单已固定；脏工作区内容不会并入。工具版本检查不是编译或模型验收。模块为文件名分组候选，后续仍需公开接口和构建验证；当前此入口不启动生成。

分析后可勾选支持的模块，调整C/C++编译器与标准，填写相对包含目录和预处理定义，再点击“保存项目输入”。保存使用分析结果的完整提交，不重新跟随分支；页面显示不可变输入版本。修改选项后须重新保存，旧输入仍保留。每个文件最多1MiB，总源码输入8MiB；缩小模块范围可降低用量。该输入供后续生成和参考评测，公开接口提取及生成操作仍待接通。
