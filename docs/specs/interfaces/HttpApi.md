...
| `GET /api/v1/runs/:runId/workflow-status` | Available | 工作流执行状态，不替代 FlywheelRun 业务状态。Console 批次详情和工作流图读取 executionStatus/currentNode/error，error仅返回与列表一致的安全原因码，原始上游错误保留在持久化审计中；失败必须显示失败阶段与原因码；读取失败显示未知，不能回退为正在运行或等待。 |
...
| `POST /api/v1/reconstructions` | Available | snapshotId、versionIds；冻结模型与实际工具链身份，启动或复用FLYWHEEL中的代码重建及接口比较。返回阶段任务；行为评测与知识修订由独立接口继续执行，重建结果本身不具有发布资格。 |
| `GET /api/v1/stage-tasks/:taskId/artifacts/:sha256` | Available | 仅下载该任务结果或成功检查点直接引用的工件；无关系的CAS摘要返回404。沿用免登录或令牌访问规则，不提供任意CAS读取。 |
| `POST /api/v1/native-evaluations` | Available | reconstructionTaskId，可选sourceVerificationTaskId及candidateSuites；后者仅在补证时接受native-supplied-candidates-v1对象（modules中的moduleId及声明式suite），拒绝可信标志或运行报告。候选按固定模块/卡片章节/接口校验并冻结CAS；候选改变形成新输入，同输入重启复用。提供的候选不调用TestGen或自动改预期，仍经参考验证和历史可信门禁。补证时冻结knowledge-test-supplement-v1及来源未知章节需求，必须绑定同一重建/版本/源/配置；绑定成功重建的结果摘要、配置、工具链与卡片版本，启动或复用EVALUATE。候选参考失败以TEST_CANDIDATE_REJECTED结束，模型提出的候选可同输入恢复生成新候选；显式提供的候选恢复时仍使用冻结内容，修改候选须形成新输入；行为失败保存报告，不授予发布资格。 |
...
- `POST /api/v1/workbench-pipelines {snapshotId, scopes?, materialIds?}`：冻结生成输入并创建或复用knowledge-pipeline-v16协调记录，返回`{pipeline}`，202或已成功时200。
...
当前修订执行使用 knowledge-revision-v5，流程使用 knowledge-pipeline-v16；v4及更早修订、v15及更早流程只读。Review 材料包含固定参考和生成代码，DocGen 只接收已标准化且绑定当前任务/原输出的纠正意见。前端风险状态仍不允许以未知归因或质量拒绝推进。
...
独立来源修订新输入冻结sourceCorrectionPolicy=source-correction-selection-v1，可选择整卡UNRESOLVED内已绑定且自身无未知风险的明确矛盾章节，原整卡风险仍保留。无policy旧输入继续原选择，未知policy拒绝；相同新policy输入复用同任务。Console为此提供来源修订与已保存/已索引版本重建入口，不把局部修订或重建标成发布通过。
...
旧检查点缺失、损坏或无法验证进程身份时，删除维护检查返回409与DELETION_CHECKPOINT_OWNER_UNKNOWN，retryable为false，中文提示需要核实旧执行者。它不同于可重试的RUNTIME_OPERATIONS_ACTIVE；不得自动填补owner、覆盖检查点或以终态批次跳过保护。
