...
源码分析、模块选择、构建参数表单、五阶段操作入口、用例失败详情和库内关系回退已接通。规范化源码诊断已接通，自动知识修订和最终发布仍待交付；不能以阶段完成代替完整产品验收。当前网站在完整验收后更新，不改写 v0.2.0。
...
生成完成后提供「执行代码重建」，展示冻结知识版本数、输入快照、累计调用、接口比较、生成代码与比较工件下载，支持取消、重启后读取和同输入恢复。可展开规范化函数差异，查看两侧变更片段、行号、词法相似度和控制关键词计数，不能将其当成语义等价。未唯一定位或预算不足明确列出；自动知识修订尚未接通；FLYWHEEL的执行成功不代表完整飞轮或发布验证通过。
...
评测结果可点击「查看修订依据」，按需校验并展示当前章节和关联失败用例。历史或失配章节、无行为观察的编译/运行故障明确列入未解决项。候选不等于已证明知识错误；查询入口只读；存在当前章节候选时可另行「执行知识修订」。修订面板显示原因、前后正文、下载、取消/恢复、质量拒绝或未解决项。索引成功且没有阻塞项后可「重建修订版本」，明确使用新版本集合。自动多轮推进仍待接通。
