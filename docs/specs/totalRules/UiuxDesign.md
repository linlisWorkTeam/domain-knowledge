...
源码分析、模块选择、构建参数表单、五阶段操作入口、用例失败详情和库内关系回退已接通。规范化源码比较、自动知识修订、外部材料和最终发布仍待交付；不能以阶段完成代替完整产品验收。当前网站在完整验收后更新，不改写 v0.2.0。
...
知识索引完成后可独立「建立关联」，显示冻结卡片数、关系数、状态、取消/恢复和关系索引下载。卡片详情提供「当前卡片不适用」读取已有关系，展示对应版本入口、实际引用行和片段、候选适用条件；明确尚未证明可替代性。无关系、过期关系、读取失败分别显示真实状态。当前仅库内符号引用关系，外部材料操作待接入，不主动联网搜索。
