...
工作台关联阶段使用冻结卡片版本建立可恢复 JSON 关系索引。首种规则为同仓库、同源码版本卡片正文对另一知识单元公开符号的明确引用；记录引用行、片段、双方版本和正文摘要。领域规则不把标签重合解释为语义依赖，也不保证关联卡片可直接替代当前卡片。没有命中返回空列表；没有外部材料明确标记仅库内关联。指定材料读取和外部关联后续接入，不主动搜索。
