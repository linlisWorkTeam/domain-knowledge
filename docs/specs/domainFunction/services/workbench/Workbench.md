...
`knowledge-pipeline-v5` 顺序推进 GENERATE、INDEX、FLYWHEEL、EVALUATE、ASSOCIATE，复用各阶段相同的prepare/start与执行器。协调记录独立于阶段执行槽，保存冻结生成输入、每个子阶段完整输入/任务身份、已完成阶段及暂停原因；不能持有阶段租约等待子阶段。子阶段先记录后启动，崩溃间隙用同幂等键补齐。成功结果复用，恢复不修改子任务输入、配置和累计预算；模型配置与生成阶段不一致时停止。取消传播至当前子任务，进程退出后显式同版本恢复。
...
修订执行设计：FLYWHEEL 的 `operation=KNOWLEDGE_REVISION`、`revisionContract=knowledge-revision-v1` 与普通代码重建分别调度。冻结评测编号、派生证据CAS、卡片版本和原模型配置。每张卡片先运行 Review，仅采纳其明确定位到候选H2的意见；未解决风险或无意见时保留原卡片。DocGen 成功后幂等提交候选版本，再调用同一索引用例刷新修改项；索引失败保留卡片和模型检查点，恢复不再次调用模型。独立执行与v4协调使用同一修订用例。
