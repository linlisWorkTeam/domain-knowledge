...
验收反例：某解析卡片写字符串 token 的 end 位于结束引号之后，而固定实现用结束引号位置；失败归因可能将本轮错误全部归于初始化并返回 UNCHANGED。最终来源门禁仍须拒绝该错误卡片，不能依赖生成代码是否采纳这句错误描述。这一反例已在独立真实模型验收中观察到；目前只新增了修订正文源码复核，完整最终门禁仍未实现。
