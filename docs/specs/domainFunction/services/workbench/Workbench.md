...
验收反例：某解析卡片写字符串 token 的 end 位于结束引号之后，而固定实现用结束引号位置；失败归因可能将本轮错误全部归于初始化并返回 UNCHANGED。最终来源门禁仍须拒绝该错误卡片，不能依赖生成代码是否采纳这句错误描述。这一反例已在独立真实模型验收中观察到；目前已接通修订正文源码复核与独立整卡复核；来源驱动修订、自动流程中的整卡门禁及最终发布事务仍未实现。
...
`knowledge-source-verification-v1` 是 EVALUATE 的独立操作，接受已完成的原生评测身份，不要求行为失败。冻结原评测的全部卡片、源码和配置，以固定源码及可信参考观察逐张检查全部 H2，包含此前未修改的卡片。Domain 判定 SOURCE_MATCHED、SOURCE_MISMATCH 或 UNRESOLVED；完整覆盖及精确正文版本绑定是聚合前提。每张卡片材料先落盘，成功检查点可复用，取消和恢复沿用阶段任务。来源匹配不等于 VERIFIED；来源意见驱动修订及最终发布事务仍待实现。
