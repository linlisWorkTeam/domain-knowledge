...
代码位置：[执行入口](../../../../../src/domain/agents/docWorkerAgent/DocWorkerAgent.ts)、[输入输出契约](../../../../../src/domain/agents/docWorkerAgent/DocWorkerAgentContract.ts)、[提示词与读取范围](../../../../../src/domain/agents/docWorkerAgent/DocWorkerAgentPrompt.ts)、[角色测试](../../../../../src/domain/agents/docWorkerAgent/DocWorkerAgent.test.ts)、[独立样例](../../../../../src/domain/agents/docWorkerAgent/examples/DocWorkerAgentSample.json)。

角色 ID：`doc-worker`。共同执行、失败和交接协议见 [Agents](../Agents.md)。

## 职责与当前输入输出

从分配给本 Worker 的源码中提取知识片段，交给 [DocGenAgent](../docGenAgent/DocGenAgent.md) 汇总。以下描述当前实现，最终业务输入输出仍待逐项确认。
