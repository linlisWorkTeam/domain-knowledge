...
工作台及配置了 `moduleContract` 的 TypeScript 模块执行显式 `section-doc-v1`；Application 传入内部 Worker 执行器，Worker 使用 `source-facts-v1` 并跨修订复用已提交证据。该契约冻结用户选定的卡片单元，先记录概要，再生成按编号对应的章节正文；不会用全文重写代替局部修订。该模式与默认项目汇总共用 DocGen 内部 Worker 交接，但不返回拆分提案。每条纠正仍验证标识、证据和文档绑定，再限制到唯一 H2；所有未授权正文及标题逐字保留，未发生修订不能返回已应用。源码页脚中独立的水平线合法，未闭合代码围栏、注入 H1/H2 和 Setext 标题均拒绝。

该契约输出的关键词来自冻结正文的 H2 标题；工作台正文工件保持原文，YAML 由索引阶段生成。默认项目流程继续生成含关键词的文档及 YAML 头，支持用户确认拆分提案。两条路径以显式命令字段区分；旧冻结任务不得跨执行版本恢复。对应回归为 `WorkbenchDocGen.test.ts`、`DocGenRevision.test.ts` 与默认 `DocGenAgent.test.ts`，全工作流与真实模型验收另行执行。
