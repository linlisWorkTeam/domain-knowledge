...
记录移除与待清理回执须原子持久化。文件清理失败返回 CLEANUP_PENDING，不能声称全部删除；进程重启后按同一回执恢复，文件适配器还须保护新增引用并完成幂等清理才可标记 DELETED。当前领域规则及应用用例已实现并用替身存储验证；SQLite引用扫描、跨库恢复、文件清理及HTTP/前台尚未接入，未对运行数据开放删除能力。

SQLite 回执适配器将传入连接内的记录移除与清理回执放在同一事务；清理失败只记录通用错误码，保留待清理状态和累计尝试次数，进程重启后可继续。已验证事务回滚和磁盘重启恢复；完整多库引用投影、跨库崩溃一致性和生产文件清理尚未实现，不能将这个单库适配器直接视为可用的删除接口。
...
SqliteDeletionSnapshot合并业务表、图内部记录和递归CAS图。先补齐业务记录指向内部检查点的引用，再递归检查CAS文件中的实体及检查点引用；未知表不得进入可删除快照。异步文件扫描前后重读记录摘要，变化则拒绝本次结果。它覆盖数据库及CAS，不能据此忽略发布Markdown、索引镜像及其他执行目录的归属。
...
删除恢复契约升级为deletion-recovery-v3，首次prepare同时冻结数据库与文件见证，持久保存文件参与者名称、契约和scope；同一意图重试不能重新capture。带命名空间的删除项必须有对应参与者，避免漏掉文件却宣告完成。文件scope绑定CAS绝对目录及设备/目录inode，替换目录或改变范围均拒绝；相对路径在构造时固定为绝对路径。旧v2意图不能跨版本恢复。
