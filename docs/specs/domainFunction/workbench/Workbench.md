...
`knowledge-source-verification-v5` 是 EVALUATE 的独立操作，接受已完成的原生评测身份，不要求行为失败。冻结原评测的全部卡片、源码和配置，以固定源码及可信参考观察逐张检查全部 H2，包含此前未修改的卡片。Domain 判定 SOURCE_MATCHED、SOURCE_MISMATCH 或 UNRESOLVED；完整覆盖及精确正文版本绑定是聚合前提。每张卡片材料先落盘，成功检查点可复用，取消和恢复沿用阶段任务。来源匹配不等于 VERIFIED；来源意见驱动修订见下方独立契约；自动来源门禁见 pipeline-v13，最终发布事务仍待实现。
...
这部分已在真实阶段/批次存储和磁盘重启测试中验证，但只验证存储执行边界。引用授权、共享执行引用墓碑、文件清理和HTTP二次确认现已串联；完整生产历史兼容仍待验收，不能以存储测试替代。旧v1删除意图不能跨契约恢复；不得将行执行器直接暴露为客户端任意删除接口。
...
旧运行已通过完整数据库引用规划到事务删除和重启读取的临时数据验证；HTTP已接入删除预览与二次确认，生产旧记录仍可能因归属证据不足而被拒绝。旧基础设施失败可能保留GENERATING/EVALUATING等业务状态，删除资格须结合真实执行状态与活跃句柄，不能将这类状态一律解释为当前运行，也不能未经验证直接放行。LangGraph检查点库已有专用库存与行清理适配器，已接入删除库存，缺少执行者身份的旧检查点仍须拒绝删除。
...
来源复核v5冻结pendingConcernsRef，保存同源码、同卡片正文版本的旧失败Review线索及原任务、事件序号和原输出摘要。线索不是已验证纠正意见；Review须按concernId逐条回应CONFIRMED、DISPROVED或UNRESOLVED。确认须保留定位修订，未决定须保留风险；否定须给出固定源码中实际存在的引用及理由，引用存在不等于系统自动证明理由成立。发布重读冻结线索并核对每节回应，不能遗漏或替换。新任务与旧v4身份分离，旧输入与用量不改写、不跨契约恢复；同v5输入重复启动复用已冻结任务，不因新增自身失败事件重建身份。运行中、失败、暂停或取消的任务仍保留原输入及累计用量；已成功任务只有新增已验证的历史矛盾时才冻结后续输入，不能因单纯新增失败线索反复创建任务。

来源格式恢复区分拒绝记录与有效输出：原始格式失败作为判断约束跨任务尝试只读传给Review；明确拒绝的事实变更继续留证，不参与可用结果比较。仅当事件和CAS工件均表明REJECTED/REVIEW_REPAIR_FACTS_CHANGED才按拒绝处理；所有接纳结果仍与原约束一致。当前尝试次数/截止时间、累计用量、任务输入及原检查点不改写，来源未解决或参考证据反驳的修订意见仍不获得发布资格。

来源结果存在明确差异时，Console提供“重新核实历史意见”，显式启动source-historical-review-v1任务，保留原批次意见和证据。该入口与普通复核和原任务恢复分开；没有逐条回应的PASS不能成为有效结果，未知问题仍显示并阻止发布。
