...
旧运行已通过完整数据库引用规划到事务删除和重启读取的临时数据验证；生产HTTP仍未接入删除。旧基础设施失败可能保留GENERATING/EVALUATING等业务状态，删除资格还须结合真实执行状态与活跃句柄，不能将这类状态一律解释为当前运行，也不能未经验证直接放行。LangGraph检查点库及其内部记录仍须纳入完整产物清理范围。

维护屏障允许在锁内异步核验执行空闲状态；核验期间拒绝其他请求及恢复操作，核验失败或状态不可用不得执行删除。核验返回后再次检查请求有效性与本地空闲状态；请求已释放不能继续删除。生产执行状态读取尚须接入该核验入口，原有保守资格检查未放宽。
