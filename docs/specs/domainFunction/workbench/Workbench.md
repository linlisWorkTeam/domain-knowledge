...
来源复核v5冻结pendingConcernsRef，保存同源码、同卡片正文版本的旧失败Review线索及原任务、事件序号和原输出摘要。线索不是已验证纠正意见；Review须按concernId逐条回应CONFIRMED、DISPROVED或UNRESOLVED。确认须保留定位修订，未决定须保留风险；否定须给出固定源码中实际存在的引用及理由，引用存在不等于系统自动证明理由成立。发布重读冻结线索并核对每节回应，不能遗漏或替换。新任务与旧v4身份分离，旧输入与用量不改写、不跨契约恢复；同v5输入重复启动复用已冻结任务，不因新增自身失败事件重建身份。
