...
工件图按内容摘要串行展开，校验每个文件的大小与摘要，并追踪JSON中的嵌套工件和实体引用；共享工件及源码依赖由同一领域规则保留。读取上限为单工件32MiB、累计256MiB和100000个节点，超限不退化为浅扫描。仅有工件标识但缺少可验证引用时明确返回未解析，不能忽略后声称清理完整。当前扫描器只用于只读审计；跨库一致快照、缺失历史工件的处理、墓碑、物理清理和删除API仍未接入，不代表前台删除已可用。
