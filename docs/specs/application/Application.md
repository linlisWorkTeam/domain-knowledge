...
发布准备对补证评测重新读取原来源任务及固定正文，派生并比较冻结需求；以当前模块的需求、目标章节和指定候选摘要重算与执行阶段相同的策略身份，不能以普通评测摘要替代。指定候选必须属于固定模块/接口/正文，并完整存在于已验证可信用例集中（允许合并时更名caseId，不允许修改其他字段）；所有原参考和生成观察门禁继续执行。


来源复核可显式选择source-historical-review-v1。该策略进入输入身份，与普通复核分别复用同版本任务及累计用量；未选择时保留原意见继承行为。新任务冻结priorFindingsRef，重读原检查点摘要、原始Review、命令和角色结果，校验同源码/正文/H2绑定；Domain将原criterion/risk转为稳定history线索。新Review必须逐条确认、反驳或保留未知，反驳需固定源码引用和理由。原始任务不改写，当前任务保留历史工件及回应；引用存在不自动证明推理正确。发布准备及已准备发布的恢复均重新验证原检查点和绑定，发布还检查原始回应与冻结线索完全一致；确认或未知仍阻止发布。显式复核不解除当前尝试中格式修正的事实保留约束。
