...
评测完成后点击「查看修订依据」，按需核对可信参考结果与固定卡片章节。页面列出可交给 Review 判断的章节及失败用例；旧章节和没有行为观察的构建故障会保留未解决项。查询入口不会修改卡片。存在候选章节时可点击「执行知识修订」，查看前后正文及原因；索引失败可恢复，已完成的模型输出保留。质量接受且索引刷新后点击「重建修订版本」，再执行评测。自动多轮推进仍待接通。
