...
浏览器打开 <http://127.0.0.1:4174>。Console 提供“操作中心、飞轮批次、知识、工作流图、评测、来源、Agent 设置”七个页面。直接本机访问可编辑，当前免登录部署也可直接操作；其他远程部署沿用配置的令牌边界。Agent 设置页面会列出七个固定角色的职责、输入输出、工具权限和基础提示词，并展示模型服务与运行/治理指标；批次详情与工作流图显示从 LangGraph 投影而来的节点状态，而不是直接读取 checkpoint 数据库。

当前源码的 C/C++ 卡片入口：先在 Agent 设置中保存并验证模型，在操作中心输入 Git 仓库根目录和提交，点击「分析仓库」，选择模块、调整编译器和包含目录后「保存项目输入」。点击「生成知识库」；类模块可展开接口范围，例如 TinyXML2 使用 `tinyxml2::XMLUtil`，限定转换符号。每张卡片完成即能阅读，暂停或取消不会删除前序卡片；「恢复生成」固定使用原输入，源码或模型配置改变后应重新启动。生成完成后到知识页面展开「知识索引与试检索」，更新索引并输入问题。生成、索引及「执行代码重建」可操作：重建提供代码下载、公开接口比较和同输入恢复。重建完成后点击「执行评测」，查看参考候选验证、可信用例通过数、失败输入/预期/实际和章节版本。候选被参考拒绝时可重新生成，累计用量保留；依赖错误先检查参考构建报告。索引完成后可独立「建立关联」；可选择已捕获的外部材料，支持取消、恢复和关系索引下载。打开卡片点击「当前卡片不适用」查看实际引用片段、关联版本及适用条件；无关联时明确显示空结果，不能把引用关系视为可直接替代。规范化源码差异可在重建结果展开，自动知识修订及最终发布门禁仍待接通，行为结果不能据此宣称已发布验证。现有 TypeScript 模块回归入口保留。
...
原生真实验收脚本 `scripts/release/RunNativeWorkbench.ts` 使用固定目标提交，依次调用生成、索引、重建、评测、关联的现有应用入口。使用 `--target jsmn|tinyxml2 --repository <本地仓库> --runtime <独立运行目录> --report <报告路径>`，失败后以 `--resume-task <原任务编号>` 保留检查点及累计用量恢复。报告区分行为通过、行为失败和未完成；关联仅来自当前库内证据，脚本成功不代表知识发布门禁通过。添加 `--mode pipeline` 可使用与操作中心相同的持久化一键入口，失败后以 `--resume-pipeline <流程编号>` 恢复。操作中心保存项目输入、确认接口范围后点击「一键执行全部」；失败时查看对应阶段产物，再按原输入恢复。v7 流程在可信行为失败后执行 Review，必要时定点修订知识并刷新索引，再重建和评测。Review 确认知识无需修改时也可使用原卡片重试 Code。迭代记录可逐轮打开评测；连续三次未减少历史最佳失败集合时暂停，恢复保留原用量。最终发布仍未接通。

外部材料先在「来源」登记允许目录中的文本文件或允许主机上的 HTTPS 地址，打开详情，填写「材料适用条件」，点击「捕获材料快照」。若内容已漂移，先刷新并确认新修订；捕获成功后可展开查看正文和材料标识。支持 UTF-8 文本、Markdown、JSON、HTML，材料最多 2 MiB；不读取 PDF/Office 二进制。捕获不会搜索其他网页，也不标记材料为已验证知识。在「知识关联」展开选材，或在操作中心「一键流程的外部材料」中选择快照；不选则只建立库内关系。卡片详情显示材料引用、来源修订与适用条件，可下载引用正文及原文。验收脚本支持 `--material-ids id1,id2`，不能与恢复参数合用，恢复保留原选材。

重建结果中的「规范化代码差异」按公开函数显示参考/生成代码变更片段及完整规范化函数体。只去除注释和空白；参数的等价内置类型写法仅用于定位，不改写函数体。相似度基于词法单元编辑距离，不能代替行为评测。若复用历史生成代码，会显示原任务并继承历史用量；旧重建输入只读，重新启动会创建含诊断契约的新任务。一键流程当前为 v3。

评测完成后点击「查看修订依据」，按需核对可信参考结果与固定卡片章节。页面列出可交给 Review 判断的章节及失败用例；旧章节和没有行为观察的构建故障会保留未解决项。查询入口不会修改卡片。存在候选章节时可点击「执行知识修订」，查看前后正文及原因；索引失败可恢复，已完成的模型输出保留。DocGen 完成后先独立核对最终正文与固定源码；拒绝时可下载草稿及复核意见，草稿不会成为新版本。复核通过、质量接受且索引刷新后点击「重建修订版本」，再执行评测。一键流程自动推进后续轮次，保留累计用量，遇到未解决风险停止。


卡片单独修订后再次点击「一键执行全部」，会为同一源码快照的当前修订版本创建新流程，复用原生成证据；界面显示冻结的修订版本。恢复已有流程则继续使用原冻结输入，不切换到之后编辑的卡片。
