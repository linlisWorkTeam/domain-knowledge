...
当前源码的 C/C++ 卡片入口：先在 Agent 设置中保存并验证模型，在操作中心输入 Git 仓库根目录和提交，点击「分析仓库」，选择模块、调整编译器和包含目录后「保存项目输入」。点击「生成知识库」；类模块可展开接口范围，例如 TinyXML2 使用 `tinyxml2::XMLUtil`，限定转换符号。每张卡片完成即能阅读，暂停或取消不会删除前序卡片；「恢复生成」固定使用原输入，源码或模型配置改变后应重新启动。生成完成后到知识页面展开「知识索引与试检索」，更新索引并输入问题。生成、索引及「执行代码重建」可操作：重建提供代码下载、公开接口比较和同输入恢复。规范化源码差异、知识修订、行为评测、关联及一键串联仍待接通，卡片不能据此宣称已经行为验证。现有 TypeScript 模块回归入口保留。
