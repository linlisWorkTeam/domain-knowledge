...
外部材料先在「来源」登记允许目录中的文本文件或允许主机上的 HTTPS 地址，打开详情，填写「材料适用条件」，点击「捕获材料快照」。若内容已漂移，先刷新并确认新修订；捕获成功后可展开查看正文和材料标识。支持 UTF-8 文本、Markdown、JSON、HTML，材料最多 2 MiB；不读取 PDF/Office 二进制。捕获不会搜索其他网页，也不标记材料为已验证知识。当前只完成材料快照，关联任务选材入口后续接通。
