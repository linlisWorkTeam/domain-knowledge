...
> 状态：R1 已合入；R2 T103/T104 的真实 DocGen、独立检查和第二工作区修改复现已通过本地验收，PR #26 已合入。实际 Run、源码复核及失败记录见 [DEV-019 证据](Status.md)。R3/R4 未执行。

## 从公共范例开始

当前公共链路已实现“Application → Domain Agent → DSH → 结果校验与工件提交”。先复现本角色样例，再修改该角色目录。共享架构见[架构设计](specs/totalRules/Architecture.md)。
...
按文件定位职责、业务分支和测试，请先查[角色开发指南](AgentDevelopment.md)。本教程负责范例运行与检查，代码分工在指南统一维护。

1. 复现范例，阅读 `docs/specs/domainFunction/agents/Agents.md` 中本角色的职责及可见材料。
...
开发者负责自己的角色，不另建 Agent 运行框架，不把固定项目的源码路径和测试命令写入公共逻辑。简单措辞调整仍可使用现有 [`promptAddon`](AgentDevelopment.md)；它不是新增工具或改变职责的入口。
...
默认使用独立运行目录 `.workpanel/docgen-example`。Linux 真实运行要求 Bubblewrap 可用，原生 DSH 模型配置方法见 [DSH 部署说明](Runtime.md)。可以使用已经安全配置的 `DEEPSEEK_API_KEY` 环境变量，或通过 Console 在同一个运行目录保存并验证模型：
...
集成测试使用明确标识的本地受控 SSE，验证生产接线、配置冻结、失败/取消和报告关联；live 结果单独记录。具体任务和阶段判定继续维护在 [DEV-019](Status.md)。
