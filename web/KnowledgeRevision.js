...
export function createKnowledgeRevisionPanel({ root, request, escapeHtml: escape, isEditable, selection, evidence, source = false }) {
  ...
  const host = () => root.querySelector(panelSelector)
  const current = () => task?.contractVersion === 'knowledge-workbench-v1' && task?.input.parameters.revisionContract === contract
  ...
  function render() {
    ...
    const eligible = source ? evidence()?.cards?.some(card => card.outcome === 'SOURCE_MISMATCH' || (card.outcome === 'UNRESOLVED' && card.sections?.some(section => section.outcome === 'SOURCE_MISMATCH' && Array.isArray(section.unresolved) && !section.unresolved.length))) : evidence()?.modules?.some(module => module.candidates.length)
    ...
    panel.innerHTML = `<h4>${source ? '来源意见修订' : '知识修订'}</h4><p>Review 判断原因，DocGen 只修改获准章节，最终正文通过独立源码复核后保存并刷新索引。修订候选仍需重新重建和评测。</p>
      ${eligible ? `<button class="primary-button" type="button" data-knowledge-revision-action="start" ${busy || active() || !isEditable() ? 'disabled' : ''}>${source ? '按来源意见修订' : '执行知识修订'}</button>` : ''}<p role="status">${escape(notice)}</p>
  ...
  root.addEventListener('click', async event => {
    if (!host() || event.target.closest(panelSelector) !== host()) return
    ...
    try {
      if (action === 'rebuild') {
        const result = await request('/api/v1/reconstructions', { method: 'POST', body: JSON.stringify({ snapshotId: task.result.summary.snapshotId, versionIds: task.result.summary.versionIds, ...(source ? {} : { retryEvaluationTaskId: task.result.summary.evaluationTaskId }) }) })
        root.dispatchEvent(new CustomEvent('workbench-reconstruction-started', { detail: result.task })); return
      }
      const result = await request(action === 'start' ? (source ? '/api/v1/source-revisions' : '/api/v1/knowledge-revisions') : `/api/v1/stage-tasks/${encodeURIComponent(task.taskId)}/${action}`, { method: 'POST', body: JSON.stringify(action === 'start' ? { [parentKey]: parent } : action === 'resume' ? { inputDigest: task.inputDigest } : {}) })
  ...
  return { refresh() {
    ...
    if (loadedFor !== id) { loadedFor = id; task = null; checkpoints = []; events = []; clearTimeout(timer); timer = null
      request('/api/v1/stage-tasks').then(result => { if (selection() !== id) return
        task = result.items.find(item => item.input.parameters.operation === operation && item.input.parameters[parentKey] === id) ?? null; return observe()
