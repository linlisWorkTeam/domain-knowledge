...
export function createKnowledgeRevisionPanel({ root, request, escapeHtml: escape, isEditable, selection, evidence, source = false }) {
  ...
  const contract = source ? 'knowledge-source-revision-v1' : 'knowledge-revision-v5'
  ...
  function render() {
    ...
    const eligible = source ? evidence()?.cards?.some(card => card.outcome === 'SOURCE_MISMATCH') : evidence()?.modules?.some(module => module.candidates.length)
