...
export function createKnowledgeRevisionPanel({ root, request, escapeHtml: escape, isEditable, selection, evidence }) {
  ...
  const current = () => task?.contractVersion === 'knowledge-workbench-v1' && task?.input.parameters.revisionContract === 'knowledge-revision-v1'
  const active = () => current() && ['PENDING', 'RUNNING'].includes(task.status)
  const reasons = { CANDIDATE_PARENT_CHANGED: '卡片已被其他任务修改，未覆盖新版本。', REVISION_SOURCE_LIMIT: '固定参考材料缺失或超出修订容量，请调整模块范围。', INDEX_BUILD_PARTIAL: '卡片已保存，部分索引未完成；恢复会继续索引。', REVISION_CARD_CHANGED: '卡片已有其他修改，请使用新版本重新评测。', REVISION_CORRECTION_OUTSIDE_EVIDENCE: 'Review 意见超出可信章节范围，未授权修改。', REVISION_NO_PROGRESS: '修订回到了已有正文，暂停以避免反复改写。', REVISION_QUALITY_REJECTED: '修订候选未通过质量检查，不能推进。', REVIEW_NO_KNOWLEDGE_CORRECTION: 'Review 没有给出知识修订意见，保留原卡片并继续诊断。' }
  ...
  function render() {
    ...
    panel.innerHTML = `<h4>知识修订</h4><p>Review 判断原因，DocGen 只修改获准章节；随后刷新相关索引。修订候选仍需重新重建和评测。</p>
  ...
  root.addEventListener('click', async event => {
    ...
    try {
      if (action === 'rebuild') {
        const result = await request('/api/v1/reconstructions', { method: 'POST', body: JSON.stringify({ snapshotId: task.result.summary.snapshotId, versionIds: task.result.summary.versionIds }) })
  ...
  return { refresh() {
    ...
    if (loadedFor !== id) { loadedFor = id; task = null; checkpoints = []; clearTimeout(timer); timer = null
