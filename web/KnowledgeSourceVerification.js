...
export function createSourceVerificationPanel({ root, request, escapeHtml: escape, isEditable, selection }) {
  ...
  const current = () => task?.contractVersion === 'knowledge-workbench-v1' && task?.input.parameters.verificationContract === 'knowledge-source-verification-v1'
  const active = () => current() && ['PENDING', 'RUNNING'].includes(task.status)
  const reasons = { AGENT_STAGE_TIMEOUT: '来源复核超时，已完成卡片和输入材料保留，可恢复原任务。', DSH_AGENT_OUTPUT_NOT_JSON: '模型输出格式错误，前序结果保留，可恢复原任务。', STAGE_PROCESS_EXITED: '上次进程退出，可恢复原任务。', PROVIDER_QUOTA_EXHAUSTED: '供应商额度不足，累计用量和已完成结果保留。' }
  ...
  function render() {
    ...
    panel.innerHTML = `<h4>整卡来源复核</h4><p>逐张检查本次评测使用的全部卡片，包括未修改的章节。行为用例通过仍需核对来源；匹配结果不代表已通过发布门禁。</p>
      ...
      ${task ? `<p>${escape({ PENDING: '排队中', RUNNING: '正在复核', SUCCEEDED: '复核执行完成', FAILED: '执行失败', PAUSED: '已暂停', CANCELLED: '已取消' }[task.status] ?? '未知')} · ${escape(outcomes[task.result?.summary.outcome] ?? '')}</p><p>累计模型调用 ${escape(task.usage.modelCalls)} 次 · ${escape(reasons[task.reasonCode] ?? task.reasonCode ?? '')}</p>
      <p>已复核 ${escape(cards.length)}/${escape(task.input.cardVersionIds.length)} 张冻结卡片。</p>
