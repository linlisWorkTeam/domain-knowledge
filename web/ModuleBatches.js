...
export function createModuleBatches({ root, request, escapeHtml: escape, project, isEditable, formatDate }) {
  ...
  function html() {
    ...
    return `<section data-module-batches>
      ...
      <div class="batch-toolbar"><button type="button" class="secondary-button" data-stage-operations>阶段操作</button><button type="button" class="secondary-button" data-batch-picker>选择批次</button><button type="button" class="secondary-button" data-recover-deletions>恢复未完成删除</button><button type="button" class="primary-button" data-new-module-batch ${isEditable() ? '' : 'disabled'}>新建批次</button></div>
      ${batch ? `<button type="button" class="secondary-button" data-delete-kind="batches" data-delete-id="${escape(batch.batchId)}" ${batch.status === 'RUNNING' || !isEditable() ? 'disabled' : ''}>删除所选批次</button>` : ''}<p role="status" data-batch-notice>${escape(notice)}</p>
