...
export function createWorkbenchPipelinePanel({ root, request, escapeHtml: escape, isEditable, selection }) {
  ...
  const active = () => pipeline && pipeline.contractVersion === 'knowledge-pipeline-v15' && ['PENDING', 'RUNNING'].includes(pipeline.status)
  ...
  const reasons = { PIPELINE_PUBLICATION_REQUIRED: '发布能力不可用，已保留阶段结果，请恢复服务后重试。', PIPELINE_PUBLICATION_BINDING_INVALID: '发布结果与本轮冻结版本不一致，未标记已验证。', PUBLICATION_DIRECTORY_CONFLICT: '发布目录内容冲突，请检查目录；前序结果已保留。', PIPELINE_FIXED_FAILED: '固定用例未通过，已保留参考与生成观察。请检查用例或重建结果。', PIPELINE_FIXED_INPUT_INVALID: '固定用例输入无效，请检查文件及模块范围。', PIPELINE_FIXED_EVALUATION_REQUIRED: '固定评测能力不可用，流程已停止。', PIPELINE_SOURCE_RESULT_INVALID: '来源复核结果未完整绑定冻结卡片，未推进。', PIPELINE_SOURCE_UNRESOLVED: '来源复核仍有未知风险，请检查固定材料和逐章意见。', PIPELINE_NO_SOURCE_PROGRESS: '连续三次未完成新的章节修订，最新版本仍有来源矛盾，已暂停。', PIPELINE_SOURCE_VERIFICATION_REQUIRED: '缺少来源复核执行器，不能跳过门禁。', PIPELINE_SOURCE_REVISION_REQUIRED: '存在源码矛盾，但缺少来源修订执行器。', PIPELINE_SOURCE_INPUT_CHANGED: '来源复核输入与本轮冻结版本不匹配。', PIPELINE_CARD_SNAPSHOT_CHANGED: '当前卡片已切换源码快照，请重新分析相应源码版本。', PIPELINE_CARD_LINEAGE_CHANGED: '当前版本无法证明来自原卡片，未自动替换输入。', PIPELINE_NO_BEHAVIOR_PROGRESS: '连续三次未消除更多已知行为失败，已暂停并保留累计用量。', PIPELINE_REVISION_QUALITY_REJECTED: '修订未通过知识质量检查，请查看候选与弱项。', PIPELINE_REVISION_UNRESOLVED: '现有证据无法支持安全修订，请查看未解决项。', PIPELINE_ENVIRONMENT_CHANGED: '工具链已变化，请启动新流程；旧流程保留冻结输入与历史产物。', PIPELINE_PROCESS_EXITED: '服务曾中断，已保存前序结果，可恢复同一流程。', PIPELINE_SHUTDOWN: '服务已停止，恢复后继续原任务。',
  ...
  function render() {
    ...
    const resumable = pipeline && ['FAILED', 'PAUSED', 'CANCELLED'].includes(pipeline.status) && pipeline.contractVersion === 'knowledge-pipeline-v15'
    panel.innerHTML = `<h3>五阶段工作台</h3><p>生成卡片、建立索引、重建代码、执行评测，再建立关联。每一步仍可独立执行。</p>
      ...
      ${pipeline.contractVersion !== 'knowledge-pipeline-v15' ? '<p>旧执行契约，只读查看历史产物；请启动新流程。</p>' : ''}${pipeline.initialVersionIds ? '<p>本流程使用启动时冻结的修订版本，原生成产物保留。</p>' : ''}<p>固定外部材料 ${(pipeline.materialIds ?? []).length} 份；固定测试模块 ${(pipeline.fixedSuites ?? []).length} 个。</p>${!pipeline.fixedSuites?.length ? '<p>此流程未提供固定用例，尚不具备最终验证发布条件。</p>' : ''}
      ...
      <ol>${Object.entries(names).map(([stage, name]) => { const task = tasks.find((item) => pipeline.children[stage] ? item.taskId === pipeline.children[stage].taskId : pipeline.contractVersion !== 'knowledge-pipeline-v15' && item.input.stage === stage)
      ...
      ${(pipeline.iterations ?? []).length ? `<h4>迭代记录</h4>${pipeline.iterations.map(round => {
        ...
        const fixed = tasks.find(item => item.taskId === round.fixedEvaluation?.taskId)
        ...
        return `<details><summary>第 ${escape(round.number)} 轮 · ${round.progress ? `通过 ${escape(round.progress.passed)}/${escape(round.progress.total)}` : '尚未取得行为结果'}</summary>
          ...
          ${fixed ? `<p>固定用例 · ${escape(statuses[fixed.status] ?? '未知')} · ${(fixed.result?.summary.modules ?? []).map(module => `${escape(module.moduleId)} ${escape(module.passed)}/${escape(module.total)}`).join('；')}</p><button type="button" class="secondary-button" data-pipeline-round="${escape(round.number)}">查看本轮固定评测</button>${(fixed.result?.artifactRefs ?? []).map(ref => `<button type="button" class="secondary-button" data-download-artifact="/api/v1/stage-tasks/${escape(fixed.taskId)}/artifacts/${escape(ref.sha256)}">下载固定评测报告</button>`).join('')}` : ''}
      ...
      ${publicationVerified && publication ? `<section><h4>已验证并发布</h4><p>${escape(publication.publicationId)}</p>${publication.files.map(file => `<button type="button" class="secondary-button" data-download-artifact="/api/v1/workbench-publications/${escape(publication.publicationId)}/artifacts/${escape(file.ref.sha256)}">下载 ${escape(file.path)}</button>`).join('')}</section>` : '<p>尚未完成验证发布。具备固定用例且各项门禁通过后，流程会提交本地发布；导出失败可恢复，前序产物保留。</p>'}` : ''}`
  }
  async function observe() {
    ...
    try { const detail = await request(`/api/v1/workbench-pipelines/${encodeURIComponent(id)}`)
      ...
      pipeline = detail.pipeline; publication = detail.publication ?? null; publicationVerified = detail.publicationVerified === true; tasks = detail.tasks; checkpoints = detail.checkpoints ?? {}; usage = detail.usage; notice = ''; render()
  ...
  root.addEventListener('change', async (event) => {
    const field = event.target.closest('[data-pipeline-fixed]'); if (!field || busy || active() || !isEditable()) return
    const file = field.files?.[0], moduleId = field.dataset.pipelineFixed, snapshotId = selection()?.snapshotId
    fixedSuites.delete(moduleId); if (!file) { render(); return }
    busy = true; notice = ''
    try {
      if (file.size > 262144) throw new Error('固定用例文件不能超过 256 KiB。')
      const suite = JSON.parse(await file.text())
      if (suite.schemaVersion !== 'native-cases-v1' || !Array.isArray(suite.cases) || !suite.cases.length || suite.cases.length > 64) throw new Error('请选择有效的固定用例文件。')
      if (selection()?.snapshotId === snapshotId) fixedSuites.set(moduleId, { suite, name: file.name })
    } catch { notice = '固定用例文件无效或超过 256 KiB，请检查后重选。' }
    finally { busy = false; render() }
  })
  root.addEventListener('click', event => { if (event.target.closest('[data-pipeline-fixed-clear]') && !busy && !active() && isEditable()) { fixedSuites.clear(); render() } })
  root.addEventListener('click', async (event) => {
    ...
    const selected = selection(); if (action === 'start' && (!selected || active() || fixedSuites.size > 0 && Object.keys(selected.scopes ?? {}).some(id => !fixedSuites.has(id)))) return
    ...
    try { const path = action === 'start' ? '/api/v1/workbench-pipelines' : `/api/v1/workbench-pipelines/${encodeURIComponent(pipeline.pipelineId)}/${action}`
      const result = await request(path, { method: 'POST', body: JSON.stringify(action === 'start' ? { ...selected, materialIds: [...selectedMaterials], fixedSuites: [...fixedSuites].map(([moduleId, value]) => ({ moduleId, suite: value.suite })) } : action === 'resume' ? { inputDigest: pipeline.inputDigest } : {}) })
      if (current !== epoch) return
      pipeline = result.pipeline; publication = null; publicationVerified = false; tasks = []; usage = null; await observe()
