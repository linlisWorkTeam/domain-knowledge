...
export function createKnowledgeReconstructionPanel({ root, request, escapeHtml: escape, isEditable, selection }) {
  ...
  function render() {
    ...
    panel.innerHTML = `<h3>代码重建与接口检查</h3><p>使用已生成卡片和公开接口重建代码。行为评测、规范化源码差异及知识修订尚未接通。</p>
      ...
      <p role="status">${escape(notice)}</p>${task ? `<p><b>${escape(task.cancelRequested && active() ? '正在取消' : labels[task.status] ?? '未知')}</b> · 尚未行为验证或发布</p>
      ...
      ${resume ? `<button class="secondary-button" type="button" data-reconstruction-action="resume" ${busy || !isEditable() ? 'disabled' : ''}>恢复重建</button>` : ''}` : ''}`
  }
