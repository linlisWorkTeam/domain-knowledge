 ...
 * 文件功能：协调知识WritingGuide用例及其依赖的领域规则与端口。
 */
...
/** 定义知识ReadabilityAssessment的数据结构与类型约束。 */
export interface KnowledgeReadabilityAssessment {
  /** 提供score信息，供调用方读取或传入。 */
  score: number;
  /** 提供formulaicPhrase数量信息，供调用方读取或传入。 */
  formulaicPhraseCount: number;
  /** 提供oversizedParagraph数量信息，供调用方读取或传入。 */
  oversizedParagraphCount: number;
}

/** 提供 assess知识Readability 对应的assess知识Readability操作。 */
