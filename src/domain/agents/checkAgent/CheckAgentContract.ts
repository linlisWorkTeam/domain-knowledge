...
export interface Finding { ruleId: string; path: string; original: string; generated: string; message: string; severity: 'BLOCKER' | 'INFO' }
...
export const outputSchema: Record<string, unknown> = {
  ...
  properties: { blocking: { type: 'boolean' }, scope: { type: 'array', minItems: 1, uniqueItems: true, items: text },
    findings: { type: 'array', items: { type: 'object', additionalProperties: false,
      required: ['ruleId', 'path', 'original', 'generated', 'message', 'severity'],
      properties: { ruleId: text, path: text, original: text, generated: text, message: text, severity: { enum: ['BLOCKER', 'INFO'] } } } } },
};
...
export function validateOutput(output: Output, input: Input): void {
  ...
  for (const finding of output.findings) {
    if (!rules.some((rule) => rule.id === finding.ruleId) || !output.scope.includes(finding.path)
