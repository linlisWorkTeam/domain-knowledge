 ...
 */
import { VERIFICATION_NEEDS, type VerificationNeed } from '../../../../knowledge/KnowledgeRisks.ts';
...
export interface Payload {
  executionContract: 'source-facts-v1';
  /** 提供模块标识信息，供调用方读取或传入。 */
  moduleId: string;
...
export function validateInput(input: Input): void {
  ...
  if (input.payload.assignedSourcePaths?.length === 0 || input.payload.moduleId !== input.moduleId) throw new Error('DOCWORKER_ASSIGNMENT_INVALID');
