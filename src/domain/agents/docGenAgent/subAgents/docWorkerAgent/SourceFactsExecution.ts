...
import { type Input, type Output, type ModelOutput, schemaFor, validateInput, resolveFacts } from './SourceFactsContract.ts';
import { validatedStage } from '../../../StageValidation.ts';
import { definition, buildPrompt, readablePaths } from './SourceFactsPrompt.ts';
...
export async function execute(input: Input, context: ExecutionContext): Promise<RoleResult<Output>> {
  ...
  }, (raw) => resolveFacts(raw as unknown as ModelOutput, input), 180_000);
