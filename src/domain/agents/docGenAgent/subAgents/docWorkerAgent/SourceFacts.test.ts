...
import { validateFacts, type Input } from './SourceFactsContract.ts';
...
test('doc-worker: normal output uses one model call and validates before returning artifacts', async () => {
  const sample = roleExample<Input>('doc-worker', 'src/domain/agents/docGenAgent/subAgents/docWorkerAgent/examples/SourceFactsSample.json');
...
test('doc-worker: cancellation before and during model execution cannot return success', async () => {
  const before = roleExample<Input>('doc-worker', 'src/domain/agents/docGenAgent/subAgents/docWorkerAgent/examples/SourceFactsSample.json'); before.controller.abort();
  await assert.rejects(execute(before.input, before.context), /AGENT_CANCELLED/);
  assert.deepEqual(before.phases, []);
  const during = roleExample<Input>('doc-worker', 'src/domain/agents/docGenAgent/subAgents/docWorkerAgent/examples/SourceFactsSample.json');
