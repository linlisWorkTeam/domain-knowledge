...
import { createArtifactRef } from '../../Domain.ts';
import test from 'node:test';
import { execute as executeDocGen } from './DocGenAgent.ts';
import type { ExecutionContext, RoleResult } from '../AgentExecution.ts';
import type { Output } from './DocGenAgentContract.ts';
async function execute(input: Input, context: ExecutionContext): Promise<RoleResult<Output>> {
  const result = await executeDocGen(input, context);
  assert.ok('body' in result.output);
  return result as RoleResult<Output>;
}
...
test('doc-gen: revision output uses one model call and validates before returning artifacts', async () => {
  const sample = roleExample<Input>('doc-gen', 'src/domain/agents/docGenAgent/examples/WorkbenchDocGenSample.json');
...
test('doc-gen: cancellation before and during model execution cannot return success', async () => {
  const before = roleExample<Input>('doc-gen', 'src/domain/agents/docGenAgent/examples/WorkbenchDocGenSample.json'); before.controller.abort();
  await assert.rejects(execute(before.input, before.context), /AGENT_CANCELLED/);
  assert.deepEqual(before.phases, []);
  const during = roleExample<Input>('doc-gen', 'src/domain/agents/docGenAgent/examples/WorkbenchDocGenSample.json');
...
test('doc-gen: initial generation validates outline before body and preserves its audit artifact', async () => {
  const sample = roleExample<Input>('doc-gen', 'src/domain/agents/docGenAgent/examples/WorkbenchDocGenSample.json');
  delete sample.input.payload.baseKnowledgeRef; sample.context.iteration = 0;
...
test('doc-gen: revision refuses to modify an unnamed section or claim an unchanged correction', async () => {
  const outside = roleExample<Input>('doc-gen', 'src/domain/agents/docGenAgent/examples/WorkbenchDocGenSample.json');
  ...
  const noChange = roleExample<Input>('doc-gen', 'src/domain/agents/docGenAgent/examples/WorkbenchDocGenSample.json');
...
test('doc-gen: H1 leakage receives a bounded field-specific repair while the outline is reused', async () => {
  const sample = roleExample<Input>('doc-gen', 'src/domain/agents/docGenAgent/examples/WorkbenchDocGenSample.json');
  delete sample.input.payload.baseKnowledgeRef; sample.context.iteration = 0; delete sample.input.payload.corrections;
...
test('doc-gen: lone CR cannot hide headings in generated or revised section content', async () => {
  for (const revision of [false, true]) {
    const sample = roleExample<Input>('doc-gen', 'src/domain/agents/docGenAgent/examples/WorkbenchDocGenSample.json');
    if (!revision) { delete sample.input.payload.baseKnowledgeRef; sample.context.iteration = 0; delete sample.input.payload.corrections; }
...
test('doc-gen: internal worker JSON retains unresolved risk and verification needs through synthesis', async () => {
  const sample = roleExample<Input>('doc-gen', 'src/domain/agents/docGenAgent/examples/WorkbenchDocGenSample.json');
  sample.input.payload.workerCount = 1;
  const content = JSON.stringify({ fragment: 'Frozen source analysis.', unresolvedRisks: ['An authorized dependency is missing'], verificationNeeds: ['MODULE_BEHAVIOR_TESTS'] });
  const ref = createArtifactRef(Buffer.from(content), 'application/json');
  const context = { ...sample.context, docWorkers: { run: async () => [{ workerId: 'worker-1', resultRef: ref,
    material: { ref, content }, unresolvedRisks: ['An authorized dependency is missing'] }] } };
  const result = await executeDocGen(sample.input, context);
  const risks = result.payload.knowledgeRisks as { kind: string; sourceRef: unknown; statement: string }[];
  assert.ok(risks.some(risk => risk.kind === 'MISSING_EVIDENCE' && risk.statement === 'An authorized dependency is missing'));
  assert.ok(risks.some(risk => risk.kind === 'MODULE_BEHAVIOR_TESTS'));
  assert.ok(risks.some(risk => JSON.stringify(risk.sourceRef) === JSON.stringify(ref)));
  assert.ok((result.payload.unresolvedRisks as string[]).includes('An authorized dependency is missing'));
  assert.deepEqual(result.payload.workerResultRefs, [ref]);
});
