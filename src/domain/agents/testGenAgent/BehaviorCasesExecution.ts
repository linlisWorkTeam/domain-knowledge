...
import { type Input, type Output, schemaFor, validateInput, validateOutput } from './BehaviorCasesContract.ts';
import { definition, buildPrompt, readablePaths } from './BehaviorCasesPrompt.ts';

/** 根据冻结材料与测试策略提出候选；参考oracle通过前不能成为可信门禁。 */
export async function execute(input: Input, context: ExecutionContext): Promise<RoleResult<Output>> {
  ...
  assertActive(context.signal);
  if ('suite' in raw && raw.oracleRequired === false) throw new Error('TEST_ORACLE_REQUIRED');
  ...
  }] : output.nativeSuite ? [{ key: 'native-case-manifest', mediaType: 'application/json',
    content: JSON.stringify({ suite: output.nativeSuite, status: 'PENDING_ORACLE', sourceSnapshotRef: input.payload.sourceSnapshotRef }, null, 2),
  }] : [];
  const payload = {
    resultKind: 'testCandidates',
    candidateSetRef: pending(output.suite ? 'candidate-tests' : output.nativeSuite ? 'native-case-manifest' : 'raw'),
    caseManifestRef: pending(output.suite ? 'case-manifest' : output.nativeSuite ? 'native-case-manifest' : 'raw'),
