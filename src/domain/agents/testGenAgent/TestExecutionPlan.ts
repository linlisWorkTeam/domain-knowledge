...
/** 相对每条命令的 cwd 解析路径；允许根内 . / ..，拒绝逃出项目。 */
export function commandPath(cwd: string | undefined, path: string): string {
  const parts: string[] = [];
  for (const value of [cwd ?? '.', path]) {
    if (/^(?:[\\/]|[A-Za-z]:)/.test(value) || value.includes('\0')) throw new Error('PROJECT_PATH_DENIED');
    for (const part of value.replaceAll('\\', '/').split('/')) {
      if (!part || part === '.') continue;
      if (part === '..') { if (!parts.length) throw new Error('PROJECT_PATH_DENIED'); parts.pop(); }
...
export function nativeTestBindings(commands: readonly Command[], suite: Output): NativeTestBinding[] {
  ...
  for (let index = 0; index < commands.length; index++) {
    ...
    const relative = (path: string) => commandPath(command.cwd, path);
    const paths = entries.filter(entry => command.args.some(arg => !arg.startsWith('-') && /\.(c|cc|cpp|cxx)$/.test(arg) && relative(arg) === entry));
    ...
    const runIndices = commands.flatMap((run, i) => i > index && run.tool === 'binary' && run.purpose === 'test'
      && commandPath(run.cwd, run.args[0] ?? '') === executable ? [i] : []);
