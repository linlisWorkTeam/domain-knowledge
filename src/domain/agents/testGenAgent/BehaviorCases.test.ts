...
import type { Input } from './BehaviorCasesContract.ts';
...
test('test-gen: normal output uses one model call and validates before returning artifacts', async () => {
  const sample = roleExample<Input>('test-gen', 'src/domain/agents/testGenAgent/examples/BehaviorCasesSample.json');
...
test('test-gen: cancellation before and during model execution cannot return success', async () => {
  const before = roleExample<Input>('test-gen', 'src/domain/agents/testGenAgent/examples/BehaviorCasesSample.json'); before.controller.abort();
  await assert.rejects(execute(before.input, before.context), /AGENT_CANCELLED/);
  assert.deepEqual(before.phases, []);
  const during = roleExample<Input>('test-gen', 'src/domain/agents/testGenAgent/examples/BehaviorCasesSample.json');
...
test('test-gen: module policy requires behavior cases instead of legacy commands', async () => {
  ...
  sample.context.model.execute = async () => ({ oracleRequired: true, candidateCommands: [{ tool: 'node', args: ['--test'] }] });
  await assert.rejects(execute(sample.input, sample.context), /AGENT_OUTPUT_INVALID/);
});


test('test-gen: a new behavior run cannot fall back to a legacy command-only policy', async () => {
  const sample = roleExample<Input>('test-gen', 'src/domain/agents/testGenAgent/examples/BehaviorCasesSample.json');
  sample.input.materials.find(({ ref }) => ref.artifactId === sample.input.payload.testPolicyRef.artifactId)!.content = {};
  await assert.rejects(execute(sample.input, sample.context), /TEST_BEHAVIOR_CONTRACT_REQUIRED/);
