...
import { nativeContract, type Input } from './BehaviorCasesContract.ts';
...
export function buildPrompt(input: Input, context: ExecutionContext): string {
  if (nativeContract(input)) return `${context.effectivePrompt}\n当前 behavior-cases-v1 仅输出宿主比较的数据用例，不输出默认项目模式的测试源码 files/cases。\n当前策略为native-cases-v1，依据授权知识正文及公开接口提出候选测试，不读取参考实现正文。返回单个完整JSON对象，先写顶层oracleRequired:true，再写nativeSuite；nativeSuite结束后只关闭外层对象，禁止在已关闭的对象外追加oracleRequired属性。必须输出完整的新套件而不是补丁。每例填写caseId、description、sections、variables、calls、observations、expected；未改动的用例也必须完整填写expected，不能通过省略expected表达沿用旧值，不能把expected写在套件顶层。sections使用知识的实际章节。变量只用公开类型和基础数值类型，数组长度1..256。所有变量默认零初始化；结构体和数组省略initial，不用整数初始化结构体。字符串参数直接写{string:文本}，不要声明const char *或其他指针变量；已有数组作为参数用read访问，单值输出参数用address取地址；调用仅限公开函数/静态方法。重载使用显式类型变量选择签名：例如变量{"name":"n","type":"int","initial":{"integer":"42"}}，调用时用{"read":{"variable":"n"}}，不要用integer字面量代替需要精确类型的重载实参。数组buf传给指针参数时用{"read":{"variable":"buf"}}，不能用address取整个数组的地址。需要检查返回值时在calls.result指定新的变量名，并从该变量观察；输出参数值和函数返回值必须分别读取，不能把同一个输出参数当作返回值。read/address为变量访问，members为字段链，index为数组下标，不提供表达式。观察必须来自目标调用的返回或受影响变量。integer/unsigned预期为精确十进制字符串，boolean为布尔，number为有限数值（相对/绝对容差1e-7），string为UTF-8文本；expected只存宿主，不写进程序。native-cases-v1没有null/nullptr参数表示，不能用boolean或整数变量假冒空指针；无法表达的调用保留为未覆盖项，不伪造测试。rejectedCandidate的constructionHints定位到具体calls实参，仅解释DSL类型语义，必须结合公开签名修复后重新验证。rejectedCandidate是本阶段已拒绝的候选及参考观察，尚未可信；用它定位错误预期或测试构造问题，解释原因后重新提出候选，所有新候选仍须参考验证。不得改写已可信用例或删除可信失败门禁。候选与参考不符时只是候选被拒绝，不能据此判定知识错误。\n受信AgentCommand：${JSON.stringify(context.command)}\n授权工件：${JSON.stringify(materialsFor(input.payload, input.materials))}`;
