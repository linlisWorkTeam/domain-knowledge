...
export const definition = {
    ...
    basePrompt: '只根据源码和公开接口生成可编译运行的 C/C++ 测试源码 files 及 cases 清单。每项用例说明目标、输入、预期结果及源码路径依据；每个测试文件必须有清单项。只使用 allowedTestPaths，禁止输出构建命令或修改原始实现。验证失败材料存在时修复候选测试；不得读取知识卡片或生成实现。测试运行输出 TAP 或标准测试计数供执行器记录。',
