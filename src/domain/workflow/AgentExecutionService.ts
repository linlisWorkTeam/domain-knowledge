 ...
 */
import { roleExecutors } from '../agents/AgentRegistry.ts';
...
export async function executeAgent(
  ...
  if (!Object.hasOwn(roleExecutors, role)) throw new Error(`AGENT_ROLE_UNKNOWN: ${role}`);
  // 各角色保留专属输入类型，类型收窄只发生在领域对外执行边界。
  const execute = roleExecutors[role].execute as unknown as (
