...
export function collectRisks(fragments: Array<{ ref: ArtifactRef; content: unknown }>): KnowledgeRisk[] {
  return fragments.flatMap(({ ref, content }) => {
    // 内部 Worker 的 CAS 片段以 JSON 字符串交接；编码不能使已记录风险消失。
    let decoded = content;
    if (typeof content === 'string' && content.trimStart().startsWith('{')) {
      try { decoded = JSON.parse(content); }
      catch { throw new Error('KNOWLEDGE_RISK_FRAGMENT_INVALID'); }
    }
    const item = decoded as { unresolvedRisks?: string[]; verificationNeeds?: VerificationNeed[] } | null;
    for (const values of [item?.unresolvedRisks, item?.verificationNeeds]) {
      if (values !== undefined && (!Array.isArray(values) || values.some(value => typeof value !== 'string' || !value.trim()))) {
        throw new Error('KNOWLEDGE_RISK_FRAGMENT_INVALID');
      }
    }
    const entries = [
