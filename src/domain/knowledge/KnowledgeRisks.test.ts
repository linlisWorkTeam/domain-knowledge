...
test('serialized worker fragments retain the same risk identities and reject malformed risk data', () => {
  const content = { unresolvedRisks: ['Missing source for an authorized branch'], verificationNeeds: ['MODULE_BEHAVIOR_TESTS'] };
  const expected = collectRisks([{ ref: source, content }]);
  assert.deepEqual(collectRisks([{ ref: source, content: JSON.stringify(content) }]), expected);
  assert.equal(expected.length, 2);
  for (const content of ['{broken', { unresolvedRisks: 'discard me' }, { verificationNeeds: [42] }]) {
    assert.throws(() => collectRisks([{ ref: source, content }]), /KNOWLEDGE_RISK_FRAGMENT_INVALID/);
  }
});
