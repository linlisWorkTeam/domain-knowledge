...
export const SOURCE_HISTORICAL_REVIEW_POLICY = 'source-historical-review-v1';
/** 原始意见仍是证据；显式复核时转换成必须逐条回应的线索，不覆盖旧记录。 */
export function historicalReviewConcern(proof: { taskId: string; checkpointKey: string; checkpointDigest: string }, finding: { versionId: string; section: string }, raw: ReviewOutput) {
  if (!raw.correction) throw new Error('SOURCE_HISTORY_BINDING_INVALID');
  return { concernId: `history-${sha256(canonicalJson(proof))}`, criterion: raw.correction.criterion, risk: raw.correction.risk,
    versionId: finding.versionId, heading: finding.section };
}
