...
import type { NodeCheckpoint } from '../../application/ports/ApplicationPorts.ts';
import type { AgentId, WorkflowNodeProjection } from '../../application/ports/ApplicationPorts.ts';

/** 定义运行评测Record的数据结构与类型约束。 */
export interface RunEvaluationRecord {
  /** 提供report信息，供调用方读取或传入。 */
  report: EvaluationReport;
  /** 提供decision信息，供调用方读取或传入。 */
  decision: GateDecision;
}

/** 定义SequencedDomain事件的数据结构与类型约束。 */
export interface SequencedDomainEvent {
  /** 提供eventSeq信息，供调用方读取或传入。 */
  eventSeq: number;
  /** 提供event信息，供调用方读取或传入。 */
  event: DomainEvent;
}
