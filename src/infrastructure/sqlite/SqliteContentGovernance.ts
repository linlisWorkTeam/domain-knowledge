 ...
 * 文件功能：提供Sqlite内容治理的基础设施实现与外部系统接入。
 */
import {
...
} from '../../application/apps/ContentGovernanceApp.ts';
import type {
...
import { structuredMarkdownDiff } from '../../domain/services/knowledge/MarkdownDiff.ts';
import {
  createPinnedHttpsDispatcher, PublicHttpsEndpointPolicy,
} from '../http/PublicHttps.ts';
import { projectActionItemObservation } from './SqliteActionItems.ts';
...
interface SourceRow {
  /** 提供sourceid信息，供调用方读取或传入。 */
  source_id: string;
  /** 提供种类信息，供调用方读取或传入。 */
  kind: string;
  /** 提供projectid信息，供调用方读取或传入。 */
  project_id: string;
  /** 提供displayname信息，供调用方读取或传入。 */
  display_name: string;
  /** 提供locator信息，供调用方读取或传入。 */
  locator: string;
  /** 提供pinnedrevision信息，供调用方读取或传入。 */
  pinned_revision: string;
  /** 提供observedrevision信息，供调用方读取或传入。 */
  observed_revision: string;
  /** 提供状态信息，供调用方读取或传入。 */
  status: string;
  /** 提供credentialref信息，供调用方读取或传入。 */
  credential_ref: string | null;
  /** 提供accesspolicyref信息，供调用方读取或传入。 */
  access_policy_ref: string;
  /** 提供recordrevision信息，供调用方读取或传入。 */
  record_revision: number;
  /** 提供lastsyncat信息，供调用方读取或传入。 */
  last_sync_at: string | null;
  /** 提供lasterrorcode信息，供调用方读取或传入。 */
  last_error_code: string | null;
  /** 提供driftjson信息，供调用方读取或传入。 */
  drift_json: string | null;
  /** 提供createdat信息，供调用方读取或传入。 */
  created_at: string;
  /** 提供updatedat信息，供调用方读取或传入。 */
  updated_at: string;
  /** 提供deletedat信息，供调用方读取或传入。 */
