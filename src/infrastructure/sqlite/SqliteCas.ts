 ...
 * 文件功能：提供SqliteCas的基础设施实现与外部系统接入。
 */
import {
...
/** 封装SQLiteFlywheelRepository的对外操作与协作依赖。 */
export class SQLiteFlywheelRepository implements FlywheelRepository {
  /** 提供database路径信息，供调用方读取或传入。 */
  readonly databasePath: string;
  /** 提供database信息，供调用方读取或传入。 */
  readonly database: DatabaseSync;
  /** 提供checkpoint租约毫秒信息，供调用方读取或传入。 */
