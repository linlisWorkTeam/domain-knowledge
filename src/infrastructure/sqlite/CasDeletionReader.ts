...
export interface DeletionFileHandle { bytes: Buffer; handle: number; parent: number; name: string;
  verified: { dev: bigint; ino: bigint; size: bigint; mtimeNs: bigint; ctimeNs: bigint } }
export class CasDeletionReader implements DeletionArtifactReader {
  ...
  protected visit<T>(artifactId: string, maxBytes: number,
    consume: (file: DeletionFileHandle) => T,
    ...
    return this.visitRelative(['sha256', digest.slice(0, 2), digest], digest, maxBytes, consume, missing);
  }
  protected visitRelative<T>(segments: string[], digest: string, maxBytes: number,
    ...
    try {
      ...
      for (const [index, name] of segments.entries()) {
        try {
          const flags = index === segments.length - 1 ? constants.O_RDONLY | constants.O_NOFOLLOW | constants.O_NONBLOCK : directoryFlags;
