...
export function modelExecutionFactory(workspaces: AgentWorkspaceProvider): ProjectWorkflowStages['modelFactory'] {
  return ({ provider, command, stage, scenario }): ModelExecutionPort => ({
    ...
    async execute(request: ModelRequest, signal?: AbortSignal) {
      ...
      return provider.run({ outputAttempts: request.outputAttempts, authorizedTools: request.tools, role: request.role, prompt: request.prompt, outputSchema: request.outputSchema,
        idempotencyKey: request.generationStep ? `${command.generationKey}:testgen-${request.generationStep}`
          : request.reportAttempt ? `${command.generationKey}:report-${request.reportAttempt}` : command.generationKey, command,
        ...
        metadata: { runId: stage.runId, nodeId: stage.nodeId, iteration: stage.iteration,
          attempt: stage.attempt, reportAttempt: request.reportAttempt ?? null, generationStep: request.generationStep ?? null,
          workerId: stage.workerId ?? null, commandId: command.commandId },
      }, signal);
    },
  });
}
